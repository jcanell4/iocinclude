<?php
/**
 * iocinclude Plugin:  Display a wiki page within another wiki page
 *
 ...
 */
class action_plugin_iocinclude extends DokuWiki_Action_Plugin {
 
    /* @var helper_plugin_iocinclude $helper */
    var $helper = null;
    ...
    public function handle_indexer_version($event, $param) {
        $event->data['plugin_iocinclude'] = '0.1.safeindex='.$this->getConf('safeindex');
    }
    ...
    public function handle_indexer(Doku_Event $event, $params) {
        ...
        // get the iocinclude metadata in order to see which pages were included
        $inclmeta = p_get_metadata($event->data['page'], 'plugin_iocinclude', METADATA_RENDER_UNLIMITED);
        ...
        if (!$all_public) { // there were non-public pages included - action required!
            ...
            // check if the tag plugin handler has already been called before the iocinclude plugin
            ...
            // restore the tag metadata if the tag plugin handler has been called before the iocinclude plugin handler.
            if ($tag_called) {
    ...
    function handle_metadata(&$event, $param) {
        ...
        if($conf['allowdebug'] && $this->getConf('debugoutput')) {
            dbglog('---- PLUGIN iocinclude META DATA START ----');
            dbglog($event->data);
            dbglog('---- PLUGIN iocinclude META DATA END ----');
        }
    }

    /**
     * Supplies the current section level to the iocinclude syntax plugin
     *
     * @author Michael Klier <chi@chimeric.de>
    ...
    function _cache_prepare(Doku_Event &$event, $param) {
        ...
        $depends = p_get_metadata($cache->page, 'plugin_iocinclude');

        if($conf['allowdebug'] && $this->getConf('debugoutput')) {
            dbglog('---- PLUGIN iocinclude CACHE DEPENDS START ----');
            dbglog($depends);
            dbglog('---- PLUGIN iocinclude CACHE DEPENDS END ----');
        }
        ...
        if (!is_array($depends['pages']) ||
            ...
            $depends['pages'] != $this->helper->_get_iocincluded_pages_from_meta_instructions($depends['instructions']) ||
            ...
            if($conf['allowdebug'] && $this->getConf('debugoutput')) {
                dbglog('---- PLUGIN iocinclude: REQUESTING CACHE PURGE ----');
                dbglog('---- PLUGIN iocinclude CACHE PAGES FROM META START ----');
                dbglog($depends['pages']);
                dbglog('---- PLUGIN iocinclude CACHE PAGES FROM META END ----');
                dbglog('---- PLUGIN iocinclude CACHE PAGES FROM META_INSTRUCTIONS START ----');
                dbglog($this->helper->_get_iocincluded_pages_from_meta_instructions($depends['instructions']));
                dbglog('---- PLUGIN iocinclude CACHE PAGES FROM META_INSTRUCTIONS END ----');
    ...
    /**
     * Handle special section edit buttons for the iocinclude plugin to get the current page
    ...
    function handle_secedit_button(Doku_Event &$event, $params) {
        ...
        if ($data['target'] == 'plugin_iocinclude_start' || $data['target'] == 'plugin_iocinclude_start_noredirect') {
            // handle the "section edits" added by the iocinclude plugin
            $fn = wikiFN($data['name']);
            ...
            array_unshift($page_stack, array(
                ...
                'redirect' => ($data['target'] == 'plugin_iocinclude_start'),
            ));
        } elseif ($data['target'] == 'plugin_iocinclude_end') {
            array_shift($page_stack);
        } elseif ($data['target'] == 'plugin_iocinclude_editbtn') {
    ...
    public function handle_move_register(Doku_Event $event, $params) {
        $event->data['handlers']['iocinclude_include'] = array($this, 'rewrite_iocinclude');
    }

    public function rewrite_iocinclude($match, $pos, $state, $plugin, helper_plugin_move_handler $handler) {
